var myInterval = null;
var QueensBrush = fabric.util.createClass(fabric.BaseBrush, {
	color: '#000',
	opacity: 0.6,
	width: 30,
	_baseWidth: 5,
	...
	initialize: function (canvas, opt) {
		...
		// this.image.src = "img/QueensBrush.png";
		// this.image.onload = () => {
		// 	this._imgload = true;
		// };
	...
	drawPolygon: function (start, end, pdiff, from, to, alpha) {
		var p1 = start.subtract(new fabric.Point(pdiff, -pdiff));
		var p2 = end.subtract(new fabric.Point(pdiff, -pdiff));
		var diff = pdiff * 2;
		this.ctx.globalAlpha = 1;
		this.ctx.lineWidth = 0.5;
		var color = fabric.util.colorValues(this.color);
		this.ctx.fillStyle = 'rgba(' + color[0] + ',' + color[1] + ',' + color[2] + ',' + alpha + ')';
		this.ctx.strokeStyle = 'rgba(' + color[0] + ',' + color[1] + ',' + color[2] + ',' + alpha * 0.5 + ')';
		this.ctx.beginPath();
		this.ctx.moveTo(p1.x + diff * from, p1.y - diff * from);
		this.ctx.lineTo(p2.x + diff * from, p2.y - diff * from);
		this.ctx.lineTo(p2.x + diff * to, p2.y - diff * to);
		this.ctx.lineTo(p1.x + diff * to, p1.y - diff * to);
		this.ctx.closePath();
		this.ctx.fill();
		this.ctx.stroke();
	},
		...
		for (var i = 0; i < this.painters.length; i++) {
			...
			if (this._drawn) {
				...
				for (j = 0; j < stepNum; j++) {
					...
					this.ctx.drawImage(this.brush._element, x - w / 2, y - h / 2, w, h);
				}

var myInterval = null;
var LightPen = fabric.util.createClass(fabric.BaseBrush, {
	color: '#000',
	opacity: 0.6,
	width: 30,
	_baseWidth: 5,
	...
	draw: function() {
        for(var i = 0; i < this.painters.length; i++) {
			...
			if (this._drawn)
			{
				...
				// this.ctx.shadowBlur = this._size * 1.5;
				this.ctx.lineWidth = this._size;
				this.ctx.globalAlpha = 1;
				this.ctx.globalCompositeOperation = 'source-over';
				// this.ctx.shadowColor = 'rgba(' + (this._color[0]) + ',' + (this._color[1]) + ',' + (this._color[2]) + ',' + (1) + ')';
